    ...
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="">Edit Test</a></li>
            </ol>
        </div>
    </div>
    <div class="row">
        <div class="col-xl-12 col-xxl-12 col-sm-12">
            <div class="card">
                <div class="card-header">
                    <div class="card-body">
                        <form role="form" method="POST" action="{{ route('teacher.test.update', $test) }}">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <h4>Details</h4>
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select id="test_type" class="form-control" name="test_type" onchange="updateQuestions(document.getElementById('chapters').value, this.value)">
                                    <option value="1" {{ $test->type ? 'selected' : '' }}>MCQ</option>
                                    <option value="0" {{ $test->type ? '' : 'selected' }}>Written</option>
                                </select>
                            </div>
                        </div>
                        <hr>
                        <div class="mb-3">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-3">
                                    ...
                                    <input type="time" class="form-control" id="duration" name="duration" value="{{ old('duration', $test->duration) }}" required>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="total_marks" class="form-control" id="total_marks" name="total_marks" value="{{ old('total_marks', $test->total_marks) }}" readonly>
                                </div>
                            </div>
                            <div class="row">
...
function updateSubjects(class_id) {
    ...
    var test = {!! json_encode($test->toArray()) !!};
        ...
        success: function(response) {
            ...
            subjects.forEach(subject => {
                ...
                if(subject.id == test.subject_id)
                    flag = true;
                subjectsSelect.options[subjectsSelect.options.length] = new Option(subject.name, subject.id, flag, flag);
            });
            updateChapters(document.getElementById('subjects').value);
...
function updateQuestions(chp_id, type) {
    ...
    // chp_id = 1;
    var ques_ids = {!! json_encode($ques_ids) !!};
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            questions.forEach(question => {
                ...
                ques_ids.forEach(ques_id => {
                    console.log(question.id + "=" + ques_id);
                    // console.log(ques_id);
                    if(question.id == ques_id)
                        flag = true;
                });
                questionsSelect.options[questionsSelect.options.length] = new Option(question.name, question.id, flag, flag);
            });
